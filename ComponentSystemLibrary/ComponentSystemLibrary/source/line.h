...
public:
	...
	bool Init(const std::vector<MyVertex>& v) {
		...
		sts = CreatePixelShader(			// ピクセルシェーダーオブジェクトを生成
			CDirectXGraphics::GetInstance()->GetDXDevice(),		// デバイスオブジェクト
			"shader/psline.hlsl",
			"main",
			"ps_5_0",
			&m_ps);
		...
		// 頂点バッファ生成
		sts = CreateVertexBufferWrite(CDirectXGraphics::GetInstance()->GetDXDevice(),
			sizeof(MyVertex), v.size(), (void*)v.data(), &m_vbuffer);
		if (!sts) {
	...
	void SetVertex(std::vector<MyVertex>& v) {
		...
		HRESULT hr = CDirectXGraphics::GetInstance()->GetImmediateContext()->Map(m_vbuffer.Get(), 0, D3D11_MAP_WRITE_DISCARD, 0, &pData);
		if (SUCCEEDED(hr)) {
			memcpy_s(pData.pData, pData.RowPitch, (void*)(v.data()), sizeof(MyVertex) * v.size());
			CDirectXGraphics::GetInstance()->GetImmediateContext()->Unmap(m_vbuffer.Get(), 0);
		}
