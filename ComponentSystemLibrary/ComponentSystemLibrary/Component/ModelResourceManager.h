...
public:
	ModelResource() { datfile = new DatFile(); }
	~ModelResource(){
		if (datfile != nullptr)
			delete datfile;
		datFileName.clear();
...
{
private:
	static std::map<unsigned int, ModelResource*> MR_Map;
...
public:
	...
	static ModelResource* GetModelResource(unsigned int _ID);		//モデルリソースの取得
