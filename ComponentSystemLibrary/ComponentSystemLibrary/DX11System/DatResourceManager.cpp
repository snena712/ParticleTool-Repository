#include "DatResourceManager.h"
#include "shaderhashmap.h"

std::map<unsigned int, DatResource*> DatResourceManager::MR_Map;

bool DatResource::CreateModelData(const char* _datfileName, const char* _vsfileName, const char* _psfileName) {
...
void DatResourceManager::Uninit() {
	UnLoadModelAll();
}

unsigned int DatResourceManager::LoadModel(const char* _datfileName, const char* _vsfileName, const char* _psfileName) {
...
void DatResourceManager::UnLoadModel(unsigned int _ID) {



}

void DatResourceManager::UnLoadModelAll() {
...
DatResource* DatResourceManager::GetModelResource(unsigned int _ID) {
	// 存在するかを確かめる
...
DatFile* DatResourceManager::GetDatFile(unsigned int _ID) {
	// 存在するかを確かめる
	auto it = MR_Map.find(_ID);
	if (it == MR_Map.end()) {
		return nullptr;
...
ID3D11VertexShader* DatResourceManager::GetVertexShader(unsigned int _ID) {
...
ID3D11PixelShader* DatResourceManager::GetPixelShader(unsigned int _ID) {
	// 存在するかを確かめる
	auto it = MR_Map.find(_ID);
	if (it == MR_Map.end()) {
		return nullptr;
...
ID3D11InputLayout* DatResourceManager::GetVertexLayout(unsigned int _ID) {
	// 存在するかを確かめる
	auto it = MR_Map.find(_ID);
	if (it == MR_Map.end()) {
		return nullptr;
...
ComPtr<ID3D11ShaderResourceView> DatResourceManager::GetTexSRV(unsigned int _ID) {
	// 存在するかを確かめる
	auto it = MR_Map.find(_ID);
	if (it == MR_Map.end()) {
		return nullptr;
